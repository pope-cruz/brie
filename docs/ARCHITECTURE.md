...
| `tasks` | `id`, `workspace_id`, `event_id`, `title`, `notes`, `assignee_membership_id?`, `due_date?`, `status`, `removed_at?`, `created_by` | Title 1–200 chars; status todo/in_progress/done; workspace matches event and assignee; date has no time |
| `schedule_segments` | `id`, `workspace_id`, `event_id`, `title`, `starts_at`, `ends_at`, `owner_membership_id?`, `instructions`, `removed_at?` | End > start; title 1–120; ownership same workspace; overlaps/out-of-event times are warnings |
