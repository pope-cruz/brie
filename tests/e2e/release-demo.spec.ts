...
test('1. owner signs in, rejects a wrong code, and creates exactly one workspace', async () => {
  ...
  await expect(ownerPage).toHaveURL(/\/app\/w\/[0-9a-f-]{36}\/events/)
  ...
  await expect(ownerPage).toHaveURL(new RegExp(`/app/w/${state.workspaceId}/events`))
...
test('5. member joins on a phone, sees no privileged navigation, and is denied direct privileged calls', async () => {
  ...
  await expect(memberPage).toHaveURL(new RegExp(`/app/w/${state.workspaceId}/events`))
  ...
  await expect(menu.getByRole('link', { name: 'Attendance' })).toHaveCount(0)
...
test('6. owner assigns the task; member completes it at 375px and it persists', async () => {
  ...
  await expect(memberPage.getByRole('button', { name: 'Set up welcome desk' })).toBeVisible()
  expect(await horizontalOverflow(memberPage)).toBeLessThanOrEqual(0)
  await memberPage.getByLabel('Status for Set up welcome desk').selectOption('done')
  await expect(memberPage.getByRole('status').filter({ hasText: 'Task status saved.' })).toBeVisible()
  await memberPage.goto(`/app/w/${state.workspaceId}/tasks?assignee=me&status=done`)
...
test('8. organizer joins, can plan and manage attendance, but cannot administer the workspace', async () => {
  ...
  await expect(organizerPage).toHaveURL(new RegExp(`/app/w/${state.workspaceId}/events`))
...
test('10. duplicate makes a clean draft; cross-event history counts each event once', async () => {
  ...
  await expect(organizerPage.getByRole('heading', { name: 'Attendance history' })).toBeVisible()
...
test('13. a task deep link with filters survives sign-out and sign-in', async () => {
  const target = `/app/w/${state.workspaceId}/tasks?status=done&assignee=anyone`
  ...
  await expect(ownerPage).toHaveURL(new RegExp(`${state.workspaceId}/tasks\\?status=done&assignee=anyone`))
  await expect(ownerPage.getByLabel('Filter by task status')).toHaveValue('done')
  await expect(ownerPage.getByLabel('Status for Set up welcome desk')).toHaveValue('done')
})
...
test('16. a dropped backend on reload shows a recoverable error, never a false onboarding screen', async () => {
  ...
  await expect(ownerPage).toHaveURL(new RegExp(`/app/w/${state.workspaceId}/events`), { timeout: 20_000 })
})
